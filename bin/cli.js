...
const createDapp = (resolvedProjectPath, projectPath) => {
	...
	const timer = setInterval(function () {
		...
		if (value >= progressBar.getTotal()) {
			clearInterval(timer);
			fse.mkdtemp(path.join(os.tmpdir(), "kava-"), (err, folder) => {
				if (err) throw err;
				execSync(
					`git clone --depth 1 ${"https://github.com/Blockchain-Prime/create-kava-dapp"} ${folder}`,
					{ stdio: "pipe" }
				);
...
const cmdLineRes = yargs(process.argv.slice(2)).command(
	"$0 [folder-name]",
	"create a new kava dApp project",
	yargs => {
		yargs.positional("folder-name", {
			describe: "Folder to create your kava dapp in",
			type: "string",
			default: null,
		});
	}
...
try {
	...
	console.log(
		chalk.magenta(`
		KAVA dApp - Blockchain Prime
`)
	);

	console.log("\n");
	console.log("🪸 Welcome to the create-kava-dapp wizard 🪸");
	...
	if (cmdLineRes.folderName) {
		...
		const finalPrompt = await prompts({
			type: "confirm",
			name: "value",
			message: `Are you sure you want to create your kava dapp in ${chalk.magenta(
				projectPath
			)} folder? \n`,
			initial: true,
